...
const AuthorityPowerCase = () => {
    ...
    const caseId = id ? `#${id.toUpperCase()}` : "#POW-1021";
    ...
    return (
        <div className="space-y-6 pb-12 max-w-5xl mx-auto">
            ...
            >
                <div className="absolute right-0 top-0 w-64 h-64 bg-amber-500/10 blur-[100px] rounded-full pointer-events-none" />
                ...
                <div className="relative z-10">
                    ...
                    <div className="flex gap-2">
                        <span className="px-3 py-1 bg-amber-500/10 text-amber-400 border border-amber-500/20 rounded-lg text-sm font-bold">Grid Failure</span>
                        <span className="px-3 py-1 bg-white/5 text-gray-300 border border-white/10 rounded-lg text-sm font-medium">Main Transformer Burst</span>
                    </div>
                </div>
            </motion.div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-6">
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-xl">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="bg-black/20 rounded-xl p-4 border border-white/5">
                                ...
                                <p className="text-white font-medium">Sector G – Power Grid Station</p>
                            </div>
                            <div className="bg-black/20 rounded-xl p-4 border border-white/5">
                                <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Submission Date</p>
                                <div className="flex items-center gap-2 text-white font-medium">
                                    <Calendar size={14} className="text-gray-400" />
                                    2024-03-22 <span className="text-gray-500 ml-1">09:15</span>
                                </div>
                            </div>
                        </div>
                    </motion.div>
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-xl">
                        ...
                        <div className="bg-amber-500/5 border border-amber-500/10 rounded-xl p-5 text-gray-300 leading-relaxed font-medium">
                            "Sudden explosion at the main transformer site. Complete power outage in Sector G and surrounding industrial blocks. Emergency containment team has isolated the area."
                        </div>
                    </motion.div>
                        ...
                        {status === "New" && (
                            <div className="space-y-4">
                                ...
                                <div className="flex gap-4">
                                    <button onClick={handleAccept} className="flex-1 py-3 bg-amber-500 hover:bg-amber-400 text-white rounded-xl font-bold shadow-lg shadow-amber-500/25 transition-all flex items-center justify-center gap-2">
                            ...
                            <div className="pt-2">
                                ...
                                <div className="bg-black/20 border border-white/5 rounded-xl p-5 mb-6">
                                    ...
                                    <select
                                        ...
                                        className="w-full appearance-none bg-black/40 border border-white/10 text-white text-sm rounded-xl px-4 py-3 pr-8 focus:outline-none focus:border-amber-500 cursor-pointer"
                                    >
                                ...
                                {status === "Resolved" && !resolutionProof && (
                                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-5 mb-6">
                                        <div className="space-y-3">
                                            <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Upload Resolution Proof</p>
                                            <p className="text-sm text-amber-400/80 mb-2">Please upload a photo showing the resolved issue (.jpg, .jpeg, .png)</p>
                                            <label className="flex flex-col items-center justify-center p-8 border-2 border-dashed border-amber-500/30 hover:border-amber-500/50 bg-black/20 rounded-xl cursor-pointer transition-colors group">
                                                <Upload size={28} className="text-amber-400/50 group-hover:text-amber-400 mb-3 transition-colors" />
                ...
                <div className="space-y-6">
                    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-xl relative overflow-hidden group">
                        ...
                        <div className="space-y-6">
                            <div>
                                <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Impact Radius</p>
                                <div className="flex items-end gap-2">
                                    <Users size={28} className="text-amber-400" />
                                    <span className="text-3xl font-black text-white leading-none">842</span>
                                    <span className="text-gray-400 font-medium mb-1">Users</span>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="text-3xl font-black text-white mb-3">92<span className="text-lg text-gray-500">/100</span></div>
                                <div className="w-full h-2 bg-white/5 rounded-full overflow-hidden">
                                    <motion.div initial={{ width: 0 }} animate={{ width: "92%" }} transition={{ duration: 1, ease: "easeOut" }} className="h-full bg-gradient-to-r from-amber-500 to-amber-300 rounded-full" />
                                </div>
                            </div>

                            <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4 flex gap-3 text-red-400">
                                <AlertTriangle size={20} className="shrink-0 mt-0.5" />
                                <p className="text-sm font-bold leading-tight pt-0.5">Emergency Power Backup Isolation Required</p>
                            </div>
                        </div>
                    </motion.div>
                </div>
