...
const lifecycleColor = (status) => {
    const map = {
        New: "text-amber-600 bg-amber-50 border-amber-200 dark:text-amber-400 dark:bg-amber-500/10 dark:border-amber-500/20",
        Accepted: "text-orange-600 bg-orange-50 border-orange-200 dark:text-orange-400 dark:bg-orange-500/10 dark:border-orange-500/20",
...
const AuthorityPowerMatrix = () => {
            ...
            try {
                ...
                if (result.success) {
                    const formatted = result.data.map(item => ({
                        ...
                        category: item.category || "Power Outage",
    ...
    return (
        <div className="space-y-6 pb-12">
            ...
            >
                <div className="absolute top-0 right-0 w-64 h-64 bg-amber-500/10 dark:bg-amber-500/5 blur-3xl -mx-20 -my-20 rounded-full" />
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                    <div>
                        <div className="flex items-center gap-3 mb-4">
                            <span className="px-3 py-1 rounded-full bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 text-[10px] font-black uppercase tracking-widest border border-amber-200 dark:border-amber-500/20">
                                Live Database
                            </span>
                            <span className="text-slate-500 dark:text-white/40 text-xs font-bold uppercase tracking-widest flex items-center gap-1.5">
                                <Server size={14} /> Power Authority
                            </span>
                        </div>
                    ...
                    <div className="text-left md:text-right">
                        ...
                        <p className="text-amber-600 dark:text-amber-400 font-bold text-xs uppercase tracking-widest mt-1">
                            Records Found
                        </p>
                    </div>
                </div>
                    ...
                    {TABS.map((tab) => (
                        <button
                            ...
                            className={`relative px-4 py-2 rounded-xl text-sm font-bold transition-all duration-300 ${activeTab === tab
                                ? "text-amber-600 bg-amber-50 dark:text-amber-400 dark:bg-amber-500/10 shadow-sm"
                ...
                <div className="flex items-center gap-3 w-full xl:w-auto">
                    <div className="relative flex-1 xl:w-64">
                        ...
                        <input
                            ...
                            className="w-full bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 text-slate-900 dark:text-white text-sm rounded-xl pl-10 pr-4 py-2.5 focus:outline-none focus:border-amber-500/50 transition-colors placeholder:text-slate-400 shadow-sm font-medium"
                        />
                    </div>
                    ...
                    <div className="relative">
                        ...
                        >
                            <div className="flex items-center gap-2 shrink-0">
                                <ListFilter size={14} className="text-amber-500 dark:text-amber-400" />
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                {loading ? (
                    <div className="flex flex-col items-center justify-center p-12 bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 rounded-3xl text-amber-500">
                        ...
                        {filteredData.length > 0 ? (
                            filteredData.map((row, i) => (
                                <motion.div
                                    ...
                                    className="group grid grid-cols-1 lg:grid-cols-12 gap-4 items-center px-6 py-5 bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 hover:border-amber-500/30 rounded-2xl transition-all shadow-sm hover:shadow-md cursor-pointer"
                                >
                                    <div className="col-span-2 flex flex-col">
                                        <span className="text-sm font-black text-slate-900 dark:text-white group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors uppercase">{row.ref}</span>
                                    ...
                                    <div className="col-span-1 flex justify-end">
                                        <div className="w-10 h-10 rounded-full bg-slate-50 dark:bg-white/5 flex items-center justify-center text-slate-400 group-hover:bg-amber-500 group-hover:text-white group-hover:shadow-lg group-hover:shadow-amber-500/25 transition-all">
                                            <ArrowRight size={18} />
                                        </div>
                                    </div>
                                </motion.div>
