  ...
  return (
    <div className="pb-16">
        ...
        {INCIDENT_BUCKET_OPTIONS.map((option) => {
          ...
          return (
            <button
              ...
              className={`text-left rounded-[1.6rem] border px-5 py-4 transition-all ${
                active
                  ? "border-emerald-400/30 bg-emerald-500/10"
                  : "border-white/10 bg-white/[0.03] hover:border-white/20"
              }`}
            >
              <p className="text-[11px] uppercase tracking-[0.22em] text-emerald-300/80 font-black">
                {option.label}
              </p>
      ...
      <AdminSurface className="p-4 md:p-5 mb-6">
        ...
        <div className="mt-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <p className="text-sm text-slate-300/75">
            ...
            <span className="font-bold text-white">{meta.totalItems || 0}</span> matching incidents.
          </p>
      ...
      {loading ? (
        ...
        <>
          <AdminSurface className="hidden lg:block overflow-hidden">
            <div className="grid grid-cols-[2.2fr_1.1fr_1.1fr_0.9fr_1.2fr_1fr] gap-4 border-b border-white/10 px-6 py-4 text-[11px] font-black uppercase tracking-[0.22em] text-slate-400">
              <span>Case</span>
              <span>Authority</span>
              <span>Status</span>
              <span>Support</span>
            ...
            {items.map((incident) => (
              <button
                ...
                className="grid w-full grid-cols-[2.2fr_1.1fr_1.1fr_0.9fr_1.2fr_1fr] gap-4 border-b border-white/10 px-6 py-5 text-left transition-colors hover:bg-white/[0.03] last:border-b-0"
              >
                <div className="min-w-0">
                  ...
                  <p className="mt-2 flex items-center gap-2 text-sm text-slate-300/75">
                    <MapPin size={14} className="text-slate-500" />
                    <span className="truncate">{incident.address || "Location unavailable"}</span>
                  </p>
                  <p className="mt-1 text-xs text-slate-500">
                    {incident.reporterName} · Reported {formatAdminDate(incident.createdAt)}
                  </p>
                </div>
                ...
                <div className="pt-1">
                  <p className="text-sm font-bold text-white">{formatAdminRelativeTime(incident.lastUpdatedAt)}</p>
              ...
              <AdminSurface key={incident.id} className="p-5">
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
                    ...
                    <AdminAuthorityBadge authority={incident.assignedAuthority} />
                    <p className="mt-2 text-xs text-slate-500">{incident.reporterName}</p>
                  </div>
                  <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
                    <p className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-400 mb-2">
                      Latest update
      ...
      <div className="mt-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <p className="text-sm text-slate-400">
          Page <span className="font-bold text-white">{meta.page || 1}</span> of{" "}
          <span className="font-bold text-white">{meta.totalPages || 1}</span>
        </p>
